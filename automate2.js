...
function joinNamesToSentence(names) {
  if (names.length === 1) {
    return `${names[0]}`
  } else if (names.length === 2) {
    return `${names[0]} and ${names[1]}`
  } else {
    // intialize a new array with all names excluding the last one
    // then join them into a string of characters separated by commas
    const newnames = names.slice(0, -1).join(', ')
    // intialize a variable for the name of the person in the last index
    const lastPerson = names[names.length - 1]
    return newnames + ' and ' + lastPerson
  }
}
...
function createDescriptionsFor(data) {
  const newData = data.map(podcast => {
    ...
    return {
      ...podcast,
      description: `${title} is a ${duration} minute ${genre} podcast hosted by ${joinNamesToSentence(hosts)}`
    }
  })
  return newData
}
