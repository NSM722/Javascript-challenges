...
const baseURL = 'https://apis.scrimba.com/emojihub/api/all/category/animals-and-nature';
const flowerBed = document.querySelector('.emoji-flower-bed');

function clearTheGarden(arr) {
   const filteredData = arr.filter(item => {
      return item.group === 'animal bug' || item.group === 'plant flower'
   })
   return filteredData
}
...
fetch(baseURL)
   ...
   .then((data) => {
      // console.log(data)
      const newData = clearTheGarden(data)
      newData.forEach(emoji => {
